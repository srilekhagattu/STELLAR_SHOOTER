...
void DrawSpaceShipLazer() {
	...
	glBegin(GL_POLYGON);           //Laser horizontal stem
...
void readFromFile() {
	...
	char ch;
	if (err == 0) {
		while (fread(&ch, sizeof(const char), 1, fp)) {
			highScore[i++] = ch;
		}
		highScore[i] = '\0';
	}
...
void writeIntoFile() {
	FILE* fp;
	errno_t err = fopen_s(&fp, "HighScoreFile.txt", "w");
	int i = 0;
	char temp[40];
	char ch;
	if (err == 0) {
		...
		if (temp[0] == '\0')
			temp[i++] = '0', temp[i++] = '\0';
		fwrite(temp, sizeof(const char) * i, 1, fp);
	}
	fclose(fp);
}
...
void keys(unsigned const char key, int x, int y)
{
...
int main(int argc, char** argv) {
	...
	FILE* fp;
	errno_t err = fopen_s(&fp, "HighScoreFile.txt", "r");      // Check if HighScoreFile.txt exists, if not, create it
	if (err == 0) {
		fclose(fp);
	}
	else {
